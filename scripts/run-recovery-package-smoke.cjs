...
async function main() {
  ...
  for (const file of [executable]) assert.equal(fs.existsSync(file), true, file);
  ...
  try {
    ...
    for (const result of verifyResults) {
      ...
      assert.equal(result.dom.version, `App v${versionParts[0]}.${versionParts[1]}.${String(versionParts[2]).padStart(2, "0")}`);
    ...
    console.log(JSON.stringify({ok:true,packageVersion,profiles:verifyResults.length,renderer:true,workbookFingerprintMatches:true,scope:"isolated packaged workflow; installer replacement is not exercised"}));
  } finally {
